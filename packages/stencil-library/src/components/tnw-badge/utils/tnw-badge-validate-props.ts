...
export function validateProps(propsValues: any[]): void {
  ...
  props.forEach((prop, i) => {
    ...
    if (isRequired && (!isNotEmptyString(value) || value === undefined || value === null)) {
      throw new Error(`tnw-badge: Required prop "${prop.name}" must have value`);
    }
    ...
    if (!isValid && value !== undefined) {
      throw new Error(`tnw-badge: Invalid prop value for "${prop.name}". Expected one of ${expectedTypes.join(", ")}, but got "${value}".`);
    }
  });
}