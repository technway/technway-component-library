...
| **largerImage** | <div>If `true`, the image will be displayed at a larger size, not be equally split with the content.</div> | `false` | `boolean` |
...
| **spacing** | <div>Controls the spacing between elements inside the card.</div> | `'sm'` | `"lg"` \| `"md"` \| `"sm"` |
