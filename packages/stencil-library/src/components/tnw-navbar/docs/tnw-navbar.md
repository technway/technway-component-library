...
| **enableSearchSlot** | <div>Enables a search slot in the navbar. When enabled, a dedicated search area will be rendered 
at the end of the navbar, positioned just before the CTA slot (if enabled). This allows for
integration of custom search functionality through slot projection.</div> | `false` | `boolean` |
| **hideMenuBelow** | <div>The breakpoint at which the navbar should be hidden. Set to `never` to never hide the navbar.</div> | `'never'` | `"1024"` \| `"1439"` \| `"567"` \| `"767"` \| `"always"` \| `"never"` |
...
| **menuData** | <div>The menu data as a JSON string. Each item should include a label, optional link, optional newTab, and optional subMenu.</div> | N/A | `string` |
...
| **search** | Slot for adding a custom search component (requires `enableSearchSlot`). |
